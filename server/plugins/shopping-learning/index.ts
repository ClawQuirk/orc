...
export class ShoppingLearningPlugin implements ServerPlugin {
  ...
  async executeTool(toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
    switch (toolName) {
      case 'shopping_learn':
        return this.learn(args as any);
      case 'shopping_recall':
        return this.recallTool(args.query as string, (args.limit as number) ?? 10);
      case 'shopping_recommend':
        return this.recommend(args.items as string[]);
      default:
  ...
  private async learn(args: {
    ...
    this.db.prepare(`
      INSERT INTO shopping_learning (id, title, content, tags, merchant, category)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(id, args.title, args.content, tags, merchant, category);
  ...
  /** Search learnings via FTS5. Returns structured results for direct use. */
  async recall(query: string, limit: number): Promise<ShoppingLearning[]> {
    if (!this.db) return [];
    try {
      const rows = this.db.prepare(`
        ...
        WHERE shopping_learning_fts MATCH ?
        ORDER BY rank
        LIMIT ?
  ...
  async recent(limit: number): Promise<ShoppingLearning[]> {
    if (!this.db) return [];
    const rows = this.db.prepare(`
  ...
  private recallTool(query: string, limit: number): Promise<ToolResult> {
    return this.recall(query, limit).then((learnings) => {
  ...
  private async recommend(items: string[]): Promise<ToolResult> {
    const lines = ['**Shopping Recommendations**', ''];
    let foundAny = false;

    for (const item of items) {
      const learnings = await this.recall(item, 3);
      if (learnings.length > 0) {
        foundAny = true;
        lines.push(`### ${item}`);
        for (const l of learnings) {
