    ...
    try {
      switch (toolName) {
        ...
        case 'projects_create': {
          const id = randomUUID();
          const ts = now();
          db.prepare('INSERT INTO projects (id, name, summary, effort_estimate, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)')
            .run(id, args.name, (args.summary as string) || null, (args.effort_estimate as string) || null, ts, ts);
